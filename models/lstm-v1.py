...
df = pd.read_csv("data/moz_13mar_dropna.csv")

df1 = pd.read_csv("data/moz_13mar_dropna.csv")

df2 = pd.read_csv("data/moz_13mar_dropna.csv")

df3 = pd.read_csv("data/moz_13mar_dropna.csv")
...
model.add(tf.keras.layers.LSTM(64, return_sequences=True, input_shape=(30,8), kernel_constraint=tf.keras.constraints.NonNeg()))
...
X_test = X_test[train_df.drop(['pred', 'target'], axis=1).columns]
...
def plot_flood_preds(res, rows, cols):
    ...
    flood_list = list(res)
    mat = np.array(flood_list).reshape(cols, rows, order='')
